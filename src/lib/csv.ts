...
/** RFC-4180-ish state machine: handles quoted fields, escaped quotes, CRLF. */
...
const normStatus = (s: string): Status =>
  (STATUSES as string[]).includes(s) ? (s as Status) : "pending";
const normCategory = (s: string): Category =>
  (CATEGORIES as string[]).includes(s) ? (s as Category) : "item";
const normAspect = (s: string): AspectKey =>
  (ASPECT_KEYS as string[]).includes(s) ? (s as AspectKey) : "16:9";
...
export function rowsFromCsv(
  ...
  for (const rec of records) {
    ...
    if (!Number.isFinite(id) || takenIds.has(id)) {
      id = nextId;
    }
    takenIds.add(id);
    nextId = id + 1;
